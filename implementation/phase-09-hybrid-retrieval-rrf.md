...
6. Define explicit behavior when one retriever returns zero results or fails.
7. Expose fused candidates for evaluation.

## Tests
Overlapping candidate lists, disjoint lists, dense-only result, sparse-only result, duplicates, deterministic tie handling, one-side failure according to configured policy.

## Failure Scenarios
Dense unavailable, sparse unavailable, both unavailable, duplicate metadata conflict, invalid candidate rank.

## Acceptance Criteria
Hybrid retrieval produces deterministic fused results and can be evaluated against dense-only and sparse-only baselines.

## Definition of Done
RRF unit tests, integration tests, degraded-mode tests, review, docs, Learning notes complete.
